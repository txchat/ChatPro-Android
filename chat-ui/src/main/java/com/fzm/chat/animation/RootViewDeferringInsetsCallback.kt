...
open class RootViewDeferringInsetsCallback(
    ...
    /**
     * 大于0，则认为是键盘打开，需要关闭panel
     */
    private var originBottom = 0

    /**
     * 底部输入框是否展开，展开情况下，不需要修改paddingBottom
     */
    ...
    override fun onApplyWindowInsets(v: View, windowInsets: WindowInsets): WindowInsets {
        ...
        originBottom = typeInsets.bottom
        val bottom = if (isExpand()) 0 else originBottom
        v.setPadding(typeInsets.left, typeInsets.top, typeInsets.right, bottom)
    ...
    override fun onEnd(animation: WindowInsetsAnimation) {
        ...
        if (originBottom > 0) {
            view?.updatePadding(bottom = originBottom)
            closePanel()
        }
    }
}
